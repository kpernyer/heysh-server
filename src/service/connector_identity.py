"""Identity Service Connector - SCI-compliant user and digital twin management.

Service Connector Interface (SCI) compliant API for managing user identities
and digital twin representations.

Key SCI principles:
- Singular resource naming: /identity/digital-twin (not /digital-twins)
- Hierarchical resources: /identity/user/{user_id}/topic
- RESTful HTTP methods: GET, POST, PUT, DELETE
- snake_case path parameters: {user_id}, {topic_id}
"""
...
class WorkingHours(BaseModel):
    ...
    start: str = Field(
        ..., pattern="^([01]?[0-9]|2[0-3]):[0-5][0-9]$", description="Start time (HH:MM)"
    )
    end: str = Field(
        ..., pattern="^([01]?[0-9]|2[0-3]):[0-5][0-9]$", description="End time (HH:MM)"
    )
    days: List[str] = Field(
        ..., description="Working days (e.g., ['monday', 'tuesday', ...])"
    )
    timezone: TimeZone
...
class CommunicationPreferences(BaseModel):
    ...
    response_time_expectation: str = Field(
        "within_24_hours", description="Expected response time"
    )
...
class DigitalTwin(BaseModel):
    ...
    current_projects: List[str] = Field(
        default_factory=list, description="Current project/topic IDs"
    )
    collaboration_notes: Optional[str] = Field(
        None, max_length=1000, description="Notes for collaborators"
    )

    # AI Assistant Settings
    ai_personality: str = Field(
        "professional", description="AI personality type for interactions"
    )
    ai_context: Dict[str, Any] = Field(
        default_factory=dict, description="Context for AI interactions"
    )
    auto_respond_enabled: bool = Field(
        False, description="Enable AI auto-responses when unavailable"
    )
...
class UserTopic(BaseModel):
    """Topic/domain where user is a member."""
...
async def get_my_digital_twin(
    ...
    Returns the authenticated user's complete digital twin representation
    including presence, work preferences, skills, and AI settings.
    """
    try:
        ...
        digital_twin = DigitalTwin(
            ...
            work_style=WorkStyle(profile_data["work_style"])
            if profile_data.get("work_style")
            else None,
            working_hours=profile_data.get("working_hours"),
            communication_preferences=CommunicationPreferences(
                **profile_data.get("communication_preferences", {})
            ),
...
async def update_my_digital_twin(
    ...
    Allows updating presence status, work preferences, skills, and other
    ...
    try:
        ...
        if (
            "presence_status" in update_data
            and update_data["presence_status"] != PresenceStatus.OFFLINE.value
        ):
        ...
        # Update profile
        update_response = supabase.table("profiles").update(update_data).eq(
            "id", user_id
        ).execute()
...
@router.get("/digital-twin", response_model=List[DigitalTwinSummary])
async def list_digital_twins(
    ...
    """List digital twins of users in your network.

    **Access Control:**
    - Only shows users who share topics with you
    """
    try:
        supabase = get_supabase_client()

        # Get current user's topics to define "network"
        user_topics = supabase.table("domain_members").select("domain_id").eq(
            "user_id", user_id
        ).execute()
        ...
        query = supabase.table("domain_members").select("user_id", "profiles:user_id(*)")
        ...
        logger.error("Failed to list digital twins", user_id=user_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/digital-twin/{target_user_id}", response_model=DigitalTwin)
async def get_digital_twin(
    ...
    **Access Control:**
    ...
    - Some private information is filtered out
    """
    try:
        supabase = get_supabase_client()

        # Check if users share any topics
        current_user_topics = supabase.table("domain_members").select("domain_id").eq(
            "user_id", user_id
        ).execute()

        current_user_topic_ids = {m["domain_id"] for m in current_user_topics.data}

        target_user_topics = supabase.table("domain_members").select("domain_id").eq(
        ...
        logger.error(
            "Failed to get user digital twin", target_user_id=target_user_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/user/{target_user_id}", response_model=UserProfile)
async def get_user_profile(
    ...
    **Access Control:**
    ...
    - Users can view profiles of users in shared topics
    """
    try:
        # Check access
        if target_user_id != user_id:
            # Check if users share topics
            supabase = get_supabase_client()
        ...
        user = await UserModel.get_user_by_id(target_user_id)

        if not user:
            logger.warning("User not found", user_id=target_user_id)
            raise HTTPException(
        ...
        logger.error("Failed to get user profile", user_id=target_user_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/user/{target_user_id}/topic", response_model=List[UserTopic])
async def list_user_topics(
    ...
    """Get topics/domains where a user is a member.

    **Access Control:**
    - Users can view their own topics
    - Users can view topics of users they share topics with
    - Only shows shared topics when viewing another user's topics
    """
    try:
        supabase = get_supabase_client()
        ...
        if target_user_id != user_id:
            ...
            shared_topic_ids = current_user_topic_ids.intersection(target_user_topic_ids)
            ...
            # Filter to only shared topics
            filter_topic_ids = list(shared_topic_ids)
        else:
            # User viewing their own topics - show all
            filter_topic_ids = None

        # Get domains where user is owner
        owned_response = supabase.table("domains").select("*").eq(
            "owner_id", target_user_id
        ).execute()

        # Get domains where user is a member
        memberships_query = supabase.table("domain_members").select(
            "domain_id, role, created_at, domains(*)"
        ).eq("user_id", target_user_id)

        if filter_topic_ids:
            memberships_query = memberships_query.in_("domain_id", filter_topic_ids)

        memberships_response = memberships_query.execute()

        # Build user topics list
        user_topics = []

        # Add owned domains
        for domain in owned_response.data or []:
            if filter_topic_ids and domain["id"] not in filter_topic_ids:
                continue

            user_topic = UserTopic(
                topic_id=domain["id"],
                topic_name=domain.get("name", "Unknown"),
                user_role="owner",
                joined_at=datetime.fromisoformat(domain["created_at"])
        ...
        # Add member domains
        seen_topic_ids = {ut.topic_id for ut in user_topics}
        for membership in memberships_response.data or []:
            domain = membership.get("domains")
            if not domain or domain["id"] in seen_topic_ids:
                continue
            ...
            user_topic = UserTopic(
                ...
                joined_at=datetime.fromisoformat(membership["created_at"])
        ...
        return user_topics[start:end]

    except HTTPException:
        raise
    except Exception as e:
        logger.error("Failed to get user topics", user_id=target_user_id, error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get user topics",
        )
